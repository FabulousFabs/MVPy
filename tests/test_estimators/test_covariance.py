...
def EM(X: np.ndarray) -> Any:
    '''
    Fixture to provide the Empirical solution from sklearn.
    '''
...
@pytest.fixture
def OAS(X: np.ndarray) -> Any:
    '''
    Fixture to provide the OAS solution from sklearn.
    '''
...
def test_OAS_numpy(X: np.ndarray, OAS: Any):
    '''
    Ensure numpy results for OAS are correct.
    '''
    
    # fit estimator
    mv_lw = mv.estimators.Covariance(method = 'oas').fit(X)
    
    # assert equality
    assert np.allclose(mv_lw.covariance_, OAS.covariance_, rtol = _ALLCLOSE_RTOL, atol = _ALLCLOSE_ATOL)
    assert np.allclose(mv_lw.shrinkage_, OAS.shrinkage_, rtol = _ALLCLOSE_RTOL, atol = _ALLCLOSE_ATOL)

def test_OAS_torch(X: np.ndarray, OAS: Any):
    '''
    Ensure torch results for OAS are correct.
    '''
    ...
    # assert equality
    assert np.allclose(mv_lw.covariance_.cpu().numpy(), OAS.covariance_, rtol = _ALLCLOSE_RTOL, atol = _ALLCLOSE_ATOL)
    assert np.allclose(mv_lw.shrinkage_.cpu().numpy(), OAS.shrinkage_, rtol = _ALLCLOSE_RTOL, atol = _ALLCLOSE_ATOL)

def test_OAS_compare_numpy_torch(X: np.ndarray):
    '''
    Make sure numpy and torch agree in their solutions.
    '''
    ...
    # fit estimators
    cov_np = mv.estimators.Covariance(method = 'oas').fit(X)
    cov_tr = mv.estimators.Covariance(method = 'oas').fit(X_tr)
    
    # assert equality
    assert np.allclose(cov_np.covariance_, cov_tr.covariance_.cpu().numpy(), rtol = _ALLCLOSE_RTOL, atol = _ALLCLOSE_ATOL)
    assert np.allclose(cov_np.shrinkage_, cov_tr.shrinkage_.cpu().numpy(), rtol = _ALLCLOSE_RTOL, atol = _ALLCLOSE_ATOL)

def test_OAS_shape_mismatch():
    '''
    Ensure we fail if shape cannot produce covariance.
    '''
    ...
    with pytest.raises(ValueError):
        mv.estimators.Covariance(method = 'oas').fit(x_np)
    
    with pytest.raises(ValueError):
        mv.estimators.Covariance(method = 'oas').fit(x_tr)
...
def test_Whitener_type_mismatch(X: np.ndarray):
    '''
    Ensure that the whitener fails if wrong datatype is provided.
    '''
