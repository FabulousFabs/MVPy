...
def test_Sliding_torch():
    ...
    # run tests
    decoder = mv.estimators.Decoder(alphas = torch.logspace(-5, 10, 20))
    model = mv.estimators.Sliding(estimator = decoder, 
                                  dims = (-1,), 
                                  n_jobs = None, 
                                  verbose = False).fit(X, y)
...
def test_Sliding_compare_numpy_torch():
    ...
    # run tests
    mod_np = mv.estimators.Sliding(estimator = mv.estimators.Decoder(alphas = np.array([1])), dims = np.array([-1]), verbose = False).fit(X_np, y_np)
    mod_tr = mv.estimators.Sliding(estimator = mv.estimators.Decoder(alphas = torch.tensor([1])), dims = torch.tensor([-1]), verbose = False).fit(X_tr, y_tr)
...
def test_Sliding_shape_mismatch():
    ...
    with pytest.raises(ValueError):
        mv.estimators.Sliding(estimator = mv.estimators.Decoder(alphas = np.array([1])), dims = np.array([-1]), verbose = False).fit(X_np, y_np)

    with pytest.raises(ValueError):
        mv.estimators.Sliding(estimator = mv.estimators.Decoder(alphas = torch.tensor([1])), dims = torch.tensor([-1]), verbose = False).fit(X_tr, y_tr)
