'''
A series of unit tests for mvpy.estimators.RidgeClassifier
'''
...
def _fit_clf(Xy: np.ndarray, backend: str = 'numpy', arguments: List[Any] = [], kwarguments: Dict[str, Any] = dict()):
    '''
    ...
    for f_i, (train, test) in enumerate(kf.split(X, L)):
        ...
        # fit and save
        clf_i = mv.estimators.RidgeClassifier(*arguments, **kwarguments).fit(
            X[train], y[train]
        )
...
def _multi_clf(f: Callable, backend: str = 'numpy', arguments: List[Any] = [], kwarguments: Dict[str, Any] = dict()):
    '''
...
@pytest.mark.parametrize(
    "f,backend,arguments,kwarguments",
    [
        # 1F2C OvR
        (_Xy_1F_2C, 'numpy', [np.logspace(-5, 10, 20)], dict(method = 'OvR')),
        (_Xy_1F_2C, 'torch', [torch.logspace(-5, 10, 20)], dict(method = 'OvR')),
        # 1F2C OvO
        (_Xy_1F_2C, 'numpy', [np.logspace(-5, 10, 20)], dict(method = 'OvO')),
        (_Xy_1F_2C, 'torch', [torch.logspace(-5, 10, 20)], dict(method = 'OvO')),
        # 1F3C OvR
        (_Xy_1F_3C, 'numpy', [np.logspace(-5, 10, 20)], dict(method = 'OvR')),
        (_Xy_1F_3C, 'torch', [torch.logspace(-5, 10, 20)], dict(method = 'OvR')),
        # 1F3C OvO
        (_Xy_1F_3C, 'numpy', [np.logspace(-5, 10, 20)], dict(method = 'OvO')),
        (_Xy_1F_3C, 'torch', [torch.logspace(-5, 10, 20)], dict(method = 'OvO')),
        # 2FnC OvR
        (_Xy_2F_nC, 'numpy', [np.logspace(-5, 10, 20)], dict(method = 'OvR')),
        (_Xy_2F_nC, 'torch', [torch.logspace(-5, 10, 20)], dict(method = 'OvR')),
        # 2FnC OvO
        (_Xy_2F_nC, 'numpy', [np.logspace(-5, 10, 20)], dict(method = 'OvO')),
        (_Xy_2F_nC, 'torch', [torch.logspace(-5, 10, 20)], dict(method = 'OvO')),
    ],
    ids = [
        # 1F2C OvR
...
def test_RidgeClassifier(f: Callable, backend: str, arguments: List[Any], kwarguments: Dict[str, Any]):
    '''
    ...
    # compute
    accuracy = _multi_clf(
        f, backend = backend, arguments = arguments, kwarguments = kwarguments
    )
