...
def test_Decoder_numpy(Xys):
    '''
    Make sure the decoder works in numpy.
    '''
    ...
    decoder = mv.estimators.Decoder(alphas = np.logspace(-5, 10, 20))
...
def test_Decoder_torch(Xys):
    '''
    Make sure the decoder works in torch.
    '''
    ...
    # run tests
    kf = sklearn.model_selection.KFold(n_splits = n_folds)
    decoder = mv.estimators.Decoder(alphas = torch.logspace(-5, 10, 20).to(torch.float64))
...
def test_Decoder_compare_numpy_torch(Xys):
    '''
    Make sure numpy and torch agree in their decoder results.
    '''
    ...
    # run tests
    kf = sklearn.model_selection.KFold(n_splits = n_folds)
    dec_np = mv.estimators.Decoder(alphas = np.logspace(-5, 10, 20))
    dec_tr = mv.estimators.Decoder(alphas = torch.logspace(-5, 10, 20).to(torch.float64))
...
def test_Decoder_shape_mismatch_fit():
    '''
    Make sure decoders throw exceptions for mismatched shapes in fit method.
    '''
    ...
    with pytest.raises(ValueError):
        mv.estimators.Decoder(alphas = np.array([1])).fit(X_np, y_np)
    
    X_tr = torch.normal(0, 1, size = (100, 5))
    y_tr = torch.normal(0, 1, size = (80, 5))
    
    with pytest.raises(ValueError):
        mv.estimators.Decoder(alphas = torch.tensor([1])).fit(X_tr, y_tr)

def test_Decoder_shape_mismatch_predict():
    '''
    ...
    dec_np = mv.estimators.Decoder(alphas = np.array([1])).fit(X_np, y_np)
    ...
    dec_tr = mv.estimators.Decoder(alphas = torch.tensor([1])).fit(X_tr, y_tr)
    
    with pytest.raises(ValueError):
        dec_tr.predict(Z_tr)

def test_Decoder_predict_without_fit():
    '''
    ...
    with pytest.raises(ValueError):
        dec_np = mv.estimators.Decoder(alphas = np.array([1])).predict(X_np)
    
    X_tr = torch.normal(0, 1, size = (100, 5))
    
    with pytest.raises(ValueError):
        dec_tr = mv.estimators.Decoder(alphas = torch.tensor([1])).predict(X_tr)
