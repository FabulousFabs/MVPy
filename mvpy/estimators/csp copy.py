...
'''
check https://github.com/devlaming/jadoc/tree/main
https://github.com/gabrieldernbach/approximate_joint_diagonalization/blob/master/phams/parpham_real_troch.py
...
def _normalise_eigenvectors_torch(vec: torch.Tensor, covs: torch.Tensor, weights: torch.Tensor) -> torch.Tensor:
    ...
    cov = (covs * weights[:,None,None]).sum(0) / weights.sum()
...
class _CSP_torch():
    ...
    def fit(self, X: torch.Tensor, y: torch.Tensor):
        ...
        covs = torch.stack(
            [
                _Empirical_torch().fit(X[mask]).covariance_ 
