...
class LabelBinariser(sklearn.base.BaseEstimator):
    ...
    classes_ : List[List[Any]]
        List including lists of class identities in y.
    N_ : int | np.ndarray | torch.Tensor
        Total number of classes (across features).
    C_ : np.ndarray | torch.Tensor
    ...
    Notes
    -----
    Note that this always creates ``n_classes`` in one-hot encodings, even when ``n_classes=2``.
    This is because, in some situations, it can be easier to handle the data when all classes
    are explicitly represented in the data.
    
    .. warning::
        Only the numpy backend supports string labels, as torch does not offer support
        for string type tensors. To avoid issues arising from this, stick to numerical
        labels unless you are certain to run analyses using only the numpy backend.
    ...
    def _get_estimator(self, y: Union[np.ndarray, torch.Tensor], *args: Any) -> sklearn.base.BaseEstimator:
        ...
        Returns
        -------
        binariser : sklearn.base.BaseEstimator
            The binariser.
        """
    ...
    def to_torch(self):
        ...
        Returns
        -------
        binariser : mvpy.estimators._LabelBinariser_torch
            The torch binariser.
        """
    ...
    def to_numpy(self):
        ...
        Returns
        -------
        binariser : mvpy.estimators._LabelBinariser_numpy
            The numpy binariser.
        """
    ...
    def clone(self) -> "LabelBinariser":
        ...
        Returns
        -------
        binariser : mvpy.estimators.LabelBinariser
            The clone.
        """
