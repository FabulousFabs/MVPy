...
def make_meeg_continuous(n_trials: int = 120, n_channels: int = 64, t_padding: float = 1.0, t_baseline: float = 0.25, t_duration: float = 1.0, t_length: float = 2.0, fs: int = 200, n_background: int = 50, n_features: int = 3, n_cycles: int = 2, snr: float = 0.1, gamma: float = 1.0, phi: float = 0.5, return_Xyß: bool = False, backend: str = 'torch', device: str = 'cpu') -> tuple[Union[np.ndarray, torch.Tensor], Union[np.ndarray, torch.Tensor]]:
    ...
    phi : float, default=0.5
        What phi to use when generating auto-correlated stimulus signals? In the AR1 process, this determines the decay of signals, where :math:`X[t+1] = \phi X[t] + E[t+1]`.
    ...
    >>> from mvpy.dataset import make_meeg_layout, make_meeg_colours, make_meeg_continuous
    >>> n_channels = 64
    >>> ch_pos = make_meeg_layout(n_channels)
    >>> ch_col = make_meeg_colours(ch_pos)
    >>> X, y = make_meeg_continuous(n_channels = n_channels)
